...
Pin the workspace pnpm release whenever package extensions are present.

**Why:** EAS can otherwise select a different pnpm release and reject an otherwise valid frozen lockfile because its computed `packageExtensionsChecksum` differs.

**How to apply:** Keep the root package-manager version aligned with the pnpm release used to generate and verify the lockfile, then run a frozen workspace install before submitting an EAS build.