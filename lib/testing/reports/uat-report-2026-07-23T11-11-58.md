...
### ✓ PRE-001 — Database connectivity (22ms)

✓ Database responds to query

### ✓ PRE-002 — Schema — all tables exist (170ms)
...
### ✓ PRE-003 — Seed data — test user exists (3ms)

✓ test@carereward.com exists in users table

### ✓ PRE-004 — Seed data — opportunities exist (92ms)

✓ At least 5 active opportunities seeded

### ✓ PRE-005 — Seed data — active redemption window exists (3ms)

✓ Exactly 1 active redemption window

### ✓ UAT-001 — Registration creates correct DB records (17ms)
...
### ✓ UAT-008 — Points transactions balance integrity (8ms)

✓ points_accounts.current_balance matches last transaction balance_after

### ✓ UAT-010 — HSA YTD does not exceed yearly limit (6ms)

✓ ytd_contribution does not exceed yearly_limit
✓ yearly_limit is $4,400

### ✓ UAT-REG — Redemptions have valid status values (4ms)
...
### ✓ UAT-020 — No user has more than 1 active insurance plan (3ms)
...
### ! UAT-016 — Audit logs exist and are populated (4ms)

**Error:** function string_agg(audit_outcome, unknown) does not exist

### ✓ UAT-016b — Audit logs have required HIPAA fields populated (9ms)
...
### ✓ UAT-018 — No user has balance exceeding earned this year (2ms)

✓ current_balance ≤ earned_this_year for all users

### ✓ UAT-019 — No orphaned records in any child table (20ms)
...
### ✓ UAT-PTYPES — All transaction types are valid (4ms)
...
### ✓ UAT-ENUM — All opportunity categories are valid (0ms)
